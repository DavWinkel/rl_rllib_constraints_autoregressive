...
class TorchCustomAutoregressiveModelS4_U1(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        if "custom_model_config" in model_config:
            ...
            if "fc_hiddens_uniform_autoreg_branches" in model_config.get("custom_model_config"):
                hiddens_uniform_action_branch = list(
                    model_config.get("custom_model_config").get("fc_hiddens_uniform_autoreg_branches", []))
        else:
            raise ValueError("Please provide custom_model_config.fcnet_hiddens_autoreg_branches")


        ## u1
        tmp_layers_action_branch_u1 = []
        pre_layer_size_action_branch_u1 = self.hidden_output_size  # prev_layer_size

        for size in hiddens_uniform_action_branch[:-1]:
            tmp_layers_action_branch_u1.append(
                SlimFC(
                    in_size=pre_layer_size_action_branch_u1,
                    out_size=size,
                    initializer=normc_initializer(1.0),
                    activation_fn=activation,
                )
            )
            pre_layer_size_action_branch_u1 = size

        # add final logit layer
        tmp_layers_action_branch_u1.append(
            SlimFC(
                in_size=pre_layer_size_action_branch_u1,  # TODO fixme
                out_size=self.action_space_dim_dict.get("0_uniform_factor")*2, #Beta distribution has two outputs # self.num_outputs_action_distr, #this was 2 before
                activation_fn=None,
                initializer=normc_initializer(0.01),
            )
        )

        self.u1_logit_layers = nn.Sequential(*tmp_layers_action_branch_u1)
        ##End u1

        ## a1
        tmp_layers_action_branch_a1=[]
        #the u1_logit is 2 but the u1_action is size 1
        pre_layer_size_action_branch_a1 = self.hidden_output_size+self.action_space_dim_dict.get("0_uniform_factor") # prev_layer_size
        ...
        pre_layer_size_action_branch_a2 = self.hidden_output_size + self.action_space_dim_dict.get("0_uniform_factor") \
                                          + self.action_space_dim_dict.get("0_allocation") #prev_layer_size
        # hiddens for the actions:
        for size in hiddens_action_branch[:-1]:
        ...
        pre_layer_size_action_branch_a3 = self.hidden_output_size + self.action_space_dim_dict.get("0_uniform_factor") +\
                                          self.action_space_dim_dict.get("0_allocation") +\
                                          self.action_space_dim_dict.get("1_allocation")  # prev_layer_size
        # hiddens for the actions:
        for size in hiddens_action_branch[:-1]:
        ...
        pre_layer_size_action_branch_a4 = self.hidden_output_size + self.action_space_dim_dict.get("0_uniform_factor")+ \
                                          self.action_space_dim_dict.get(
    ...
    def forward_action_model_u1(self, ctx_input):

        u1_logit = self.u1_logit_layers(ctx_input)

        #inf_mask = torch.clamp(torch.log(self.current_action_masks.get('0')), min=FLOAT_MIN)

        return u1_logit

    def forward_action_model_a1(self, ctx_input, u1_input):

        a1_cat_input = torch.cat([ctx_input, u1_input], 1)

        a1_logit = self.a1_logit_layers(a1_cat_input)
    ...
    def forward_action_model_a2(self, ctx_input, u1_input, a1_action):

        a2_cat_input = torch.cat([ctx_input, u1_input, a1_action], 1)
    ...
    def forward_action_model_a3(self, ctx_input, u1_input, a1_action, a2_action):

        a3_cat_input = torch.cat([ctx_input, u1_input, a1_action, a2_action], 1)
    ...
    def forward_action_model_a4(self, ctx_input, u1_input, a1_action, a2_action, a3_action):

        a4_cat_input = torch.cat([ctx_input, u1_input, a1_action, a2_action, a3_action], 1)
