...
class TorchAutoregressiveDirichletDistributionS4(TorchDistributionWrapper):
    ...
    def __init__(self, inputs, model):
        ...
        ###
        self.last_sample = TorchAutoregressiveDirichletDistributionS4.merge_action(self.model,
                                                                                      action_a1=self.last_sample_a1,
                                                                                      action_a2=self.last_sample_a2,
                                                                                      action_a3=self.last_sample_a3,
    ...
    def deterministic_sample(self) -> TensorType:
        ...
        return TorchAutoregressiveDirichletDistributionS4.action_output_wrapper(self.model, self.last_sample)
    ...
    def logp(self, action):
        ...
        #slice actions into different aspects
        dict_actions = TorchAutoregressiveDirichletDistributionS4.action_output_wrapper(self.model, action)
    ...
    def action_output_wrapper(model, action):
        list_actions = TorchAutoregressiveDirichletDistributionS4.slice_action(model, action)
    ...
    @staticmethod
    def merge_action(model, action_a1, action_a2, action_a3, action_a4):
        list_actions = []
        ...
        if isinstance(action_a1, torch.Tensor):
...
class TorchAutoregressiveDirichletDistributionS4_U1(TorchDistributionWrapper):
    ...
    def __init__(self, inputs, model):
        ...
        concentration_u1 = torch.exp(u1_logit) + self.epsilon

        self.dist_u1 = torch.distributions.dirichlet.Dirichlet(
            concentration=concentration_u1,
            validate_args=True,
        )
        tmp_sample = self.dist_u1.sample()
        list_u1_logit = torch.tensor_split(tmp_sample, 2, dim=1)
        self.last_sample_u1 = list_u1_logit[0]
        """
        u1_logit = self.model.forward_action_model_u1(self.inputs)
    ...
    def deterministic_sample(self) -> TensorType:
        epsilon = torch.tensor(1e-7).to(self.inputs.device)
        u1_logit = self.model.forward_action_model_u1(self.inputs)
        concentration_u1 = torch.exp(u1_logit) + epsilon
        tmp_sample = nn.functional.softmax(concentration_u1, dim=1)
        list_u1_logit = torch.tensor_split(tmp_sample, 2, dim=1)
        self.last_sample_u1 = list_u1_logit[0]
        """
        u1_logit = self.model.forward_action_model_u1(self.inputs)
    ...
    def logp(self, action):
        ...
        action_u1_1_part = dict_actions.get("0_uniform_factor")
        action_u1_2_part = torch.ones_like(action_u1_1_part)-action_u1_1_part
        action_u1 = torch.cat([action_u1_1_part, action_u1_2_part], 1)
